pub mod rsa {
    ...
    pub fn encrypt_data(data: Vec<u8>, public_key: RsaPublicKey) -> Result<Vec<u8>, rsa::errors::Error> {
    ...
    pub fn encrypt_data_file(file_path: impl AsRef<Path>, public_key_path: impl AsRef<Path>) -> Result<Vec<u8>, rsa::errors::Error> {
...
mod rsa_test {
    ...
    #[cfg(test)]
    mod encrypts_data {
        use crate::{encrypter::rsa::{encrypt_data, encrypt_data_file}, key::rsa::get_public_key};
        ...
        fn encrypts_raw_data() {
            ...
            let encrypted_data = encrypt_data(unencrypted_data.clone(), get_public_key(key_path)).unwrap();
            assert_ne!(unencrypted_data.iter().count(), encrypted_data.iter().count());
            let encrypted_file_bytes = std::fs::read("test-files/encrypted-files/test-read-file.encrypted").unwrap();
